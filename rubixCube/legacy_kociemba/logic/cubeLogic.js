...
const FACELET_EXPORT_ORDER = ['top', 'right', 'front', 'bottom', 'left', 'back'];
...
function faceIndexToMeta(face, index) {
...
const FACELET_METADATA = FACELET_EXPORT_ORDER.flatMap((face) => Array.from({ length: 9 }, (_, index) => faceIndexToMeta(face, index)));
const FACELET_INDEX_BY_KEY = new Map(FACELET_METADATA.map((meta, index) => [metaKey(meta), index]));
function createQuarterTurnPermutation(move) {
    const { axis, layer, direction } = MOVE_ROTATIONS[move];
    const permutation = Array(54).fill(0);
...
const QUARTER_TURN_PERMUTATIONS = {
...
export const MOVE_FACELET_PERMUTATIONS = {
    U: QUARTER_TURN_PERMUTATIONS.U,
    "U'": composePermutation(QUARTER_TURN_PERMUTATIONS.U, composePermutation(QUARTER_TURN_PERMUTATIONS.U, QUARTER_TURN_PERMUTATIONS.U)),
    U2: composePermutation(QUARTER_TURN_PERMUTATIONS.U, QUARTER_TURN_PERMUTATIONS.U),
    D: QUARTER_TURN_PERMUTATIONS.D,
    "D'": composePermutation(QUARTER_TURN_PERMUTATIONS.D, composePermutation(QUARTER_TURN_PERMUTATIONS.D, QUARTER_TURN_PERMUTATIONS.D)),
    D2: composePermutation(QUARTER_TURN_PERMUTATIONS.D, QUARTER_TURN_PERMUTATIONS.D),
    L: QUARTER_TURN_PERMUTATIONS.L,
    "L'": composePermutation(QUARTER_TURN_PERMUTATIONS.L, composePermutation(QUARTER_TURN_PERMUTATIONS.L, QUARTER_TURN_PERMUTATIONS.L)),
    L2: composePermutation(QUARTER_TURN_PERMUTATIONS.L, QUARTER_TURN_PERMUTATIONS.L),
    R: QUARTER_TURN_PERMUTATIONS.R,
    "R'": composePermutation(QUARTER_TURN_PERMUTATIONS.R, composePermutation(QUARTER_TURN_PERMUTATIONS.R, QUARTER_TURN_PERMUTATIONS.R)),
    R2: composePermutation(QUARTER_TURN_PERMUTATIONS.R, QUARTER_TURN_PERMUTATIONS.R),
    F: QUARTER_TURN_PERMUTATIONS.F,
    "F'": composePermutation(QUARTER_TURN_PERMUTATIONS.F, composePermutation(QUARTER_TURN_PERMUTATIONS.F, QUARTER_TURN_PERMUTATIONS.F)),
    F2: composePermutation(QUARTER_TURN_PERMUTATIONS.F, QUARTER_TURN_PERMUTATIONS.F),
    B: QUARTER_TURN_PERMUTATIONS.B,
    "B'": composePermutation(QUARTER_TURN_PERMUTATIONS.B, composePermutation(QUARTER_TURN_PERMUTATIONS.B, QUARTER_TURN_PERMUTATIONS.B)),
...
function faceletsToCube(stickers) {
    const cube = {};
...
function applyStickerPermutation(cube, permutation) {
...
export function moveToQuarterTurns(move) {
    if (move.endsWith('2')) {
        return 2;
    }
    return move.endsWith("'") ? 3 : 1;
}
function quarterTurnsToMoves(face, turns) {
...
export function expandMove(move) {
    if (move.endsWith('2')) {
        const quarterTurn = move[0];
...
export function compressMovesToHTM(moves) {
    const optimized = [];
...
export function generateScrambleSequence(moveCount = Math.floor(Math.random() * 13) + 8) {
...
export function isSolved(cube) {
    return Object.values(cube).every((face) => face.every((color) => color === face[0]));
}
export function scrambleCube(cube, moveCount = Math.floor(Math.random() * 13) + 8) {
    return applyMoves(cube, generateScrambleSequence(moveCount));
}
export function getHint(cube) {
    if (isSolved(cube)) {
